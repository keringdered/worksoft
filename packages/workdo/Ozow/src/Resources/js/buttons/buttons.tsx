...
export const paymentMethodBtn = (data?: any) => {
    ...
    const ozowEnabled = getAdminSetting('ozow_enabled');

    if (ozowEnabled === 'on') {
        return [{
            id: 'ozow-payment',
            dataUrl: route('ozow.plan.pay'),
            ...
            component: (
                <div className="flex items-center space-x-3 p-3 border border-gray-200 dark:border-gray-700 rounded-lg w-full">
                    ...
                    <Label htmlFor="ozow" className="cursor-pointer flex items-center space-x-2">
                        <div>
                            <div className="font-medium text-gray-900 dark:text-white">{t('Ozow')}</div>
                        </div>
                        <img src={getPackageFavicon('Ozow')} alt="Ozow" className="h-10 w-10" />
                    </Label>
                </div>
            )
        }];
