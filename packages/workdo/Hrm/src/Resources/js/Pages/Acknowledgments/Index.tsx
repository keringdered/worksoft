            ...
            render: (_: any, ackItem: Acknowledgment) => (
                <div className="flex gap-1">
                    <TooltipProvider>
                        ...
                        {ackItem.document?.document && ackItem.status === 'acknowledged' && auth.user?.permissions?.includes('download-acknowledgment') && (
                            <Tooltip delayDuration={0}>
                                <TooltipTrigger asChild>
                                        ...
                                        <Card key={acknowledgment.id} className="p-0 hover:shadow-lg transition-all duration-200 relative overflow-hidden flex flex-col h-full min-w-0">
                                            ...
                                            <div className="flex justify-end gap-2 p-3 border-t bg-gray-50/50 flex-shrink-0 mt-auto">
                                                <TooltipProvider>
                                                    ...
                                                    {auth.user?.permissions?.includes('download-acknowledgment') && acknowledgment.status === 'acknowledged' && acknowledgment.document?.document && (
                                                        <Tooltip delayDuration={300}>
                                                            <TooltipTrigger asChild>
                                                                <Button variant="ghost" size="sm" asChild className="h-9 w-9 p-0 text-orange-600 hover:text-orange-700">
                                                                    <a href={getImagePath(acknowledgment.document?.document)} download>
                                                                        <Download className="h-4 w-4" />
                                                                    </a>
                                                                </Button>
                                                            </TooltipTrigger>
