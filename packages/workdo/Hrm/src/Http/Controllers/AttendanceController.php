...
class AttendanceController extends Controller
{
    public function index()
    {
        if (Auth::user()->can('manage-attendances')) {
            ...
            $defaultMonth = ($isDemo && !request()->has('month')) ? 7 : now()->month;
            $defaultYear  = ($isDemo && !request()->has('year'))  ? 2025 : now()->year;
            ...
            if (request('employee_id') && request('employee_id') !== 'all') {
                ...
                if (is_numeric($employeeId)) {
                    $employeeQuery->where('user_id', $employeeId);
                } else {
                    $employeeQuery->whereHas('user', function ($q) use ($employeeId) {
                        $q->where('name', 'like', '%' . $employeeId . '%');
                    });
                }
            }

            $usersQuery = User::emp()->where('created_by', creatorId())
                ->whereIn('id', $employeeQuery->pluck('user_id'));

            $perPage = (int) (request('per_page') ?? 10);
            $employees = $usersQuery->select('id', 'name', 'avatar')
                ->paginate($perPage);

            $employeeCollection = $employees->getCollection();
            $allEmployeeIds = (clone $usersQuery)->pluck('id');

            $records = Attendance::whereIn('employee_id', $allEmployeeIds)
            ...
            $holidays = Holiday::where('start_date', '<=', $endDate->toDateString())
                ->where('end_date', '>=', $startDate->toDateString())
                ->where('created_by', creatorId())
                ->get();

            $globalSettings = getCompanyAllSetting(creatorId());
            ...
            for ($d = 1; $d <= $daysInMonth; $d++) {
                ...
                foreach ($holidays as $holiday) {
                    if (Carbon::parse($holiday->start_date)->lte($date) && Carbon::parse($holiday->end_date)->gte($date)) {
                        $isHoliday = true;
            ...
            $employeeModels = Employee::whereIn('user_id', $employeeCollection->pluck('id'))
                ->with('shift', 'designation')
                ->get()
                ->keyBy('user_id');

            $employeeRows = $employeeCollection->map(function ($user) use ($records, $leaves, $dayHeaders, $year, $month, $employeeModels) {
                $avatarUrl = $user->avatar ?: '';
                $empModel = $employeeModels->get($user->id);
                ...
                foreach ($dayHeaders as $header) {
                    ...
                    if (isset($records[$key])) {
                        ...
                        $statusStr = strtolower($record->status);
                        
                        if ($statusStr === 'present') {
                            $presentDays++;
                        }
                        if ($statusStr === 'half day' || $statusStr === 'half_day') {
                            $presentDays += 0.5;
                            $statusStr = 'half_day';
                        }
                        ...
                        if ($empModel && $empModel->getRelation('shift')) {
                            ...
                            if ($shiftRel->end_time < $shiftRel->start_time) {
                                $shiftEndTime->addDay();
                            }
                            
                            if ($record->clock_in) {
                                $actualClockIn = Carbon::parse($record->clock_in)->setDate($record->date->year, $record->date->month, $record->date->day);
                                if ($actualClockIn->gt($shiftStartTime)) {
                                    $isLate = true;
                                }
                            }
                            if ($record->clock_out) {
                                $actualClockOut = Carbon::parse($record->clock_out)->setDate($record->date->year, $record->date->month, $record->date->day);
                                if ($actualClockOut->lt($shiftEndTime)) {
                                    $isEarly = true;
                                }
                            }
                        }
                        ...
                        $days[] = [
                            ...
                            'total_hours' => $record->total_hour,
                            'break_hours' => $record->break_hour,
                            'notes' => $record->notes,
                            'is_holiday' => $header['is_holiday'],
                    ...
                    $onLeave = $leaves->first(function ($leave) use ($user, $date) {
                    ...
                    if ($onLeave) {
                        $days[] = [
                            ...
                            'overtime_hours' => 0,
                            'is_weekend' => false,
                            'leave_type_name' => $onLeave->leave_type ? $onLeave->leave_type->title : 'Leave',
                        ];
                        continue;
                    }

                    if ($header['is_holiday']) {
                        $days[] = ['status' => 'holiday', 'is_late' => false, 'is_early_departure' => false, 'overtime_hours' => 0, 'is_weekend' => false];
                        continue;
                    }
                ...
                return [
                    'id' => $user->id,
                    'name' => $user->name,
                    'designation' => $empModel?->getRelation('designation')?->designation_name,
                    'shift' => $empModel?->getRelation('shift')?->shift_name,
                    'avatar' => $avatarUrl,
                    'days' => $days,
