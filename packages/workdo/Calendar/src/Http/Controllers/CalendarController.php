        ...
        if (Auth::user()->can('manage-calendar')) {
            ...
            if ($calendarType == 'google') {
                try {
                    ...
                    return redirect()->back()->with('error', __('Failed to fetch Google Calendar events'));
                }
            }
                    ...
                    if ($moduleFilter == "contract_end" || $moduleFilter == 'all') {
                        ...
                        foreach ($contracts as $contract) {
                            $events[] = [
                                ...
                                'start' => $contract->end_date ? $contract->end_date->format('Y-m-d') : ($contract->start_date ? $contract->start_date->format('Y-m-d') : null),
                                'end' => $contract->end_date ? $contract->end_date->format('Y-m-d') : ($contract->start_date ? $contract->start_date->format('Y-m-d') : null),
                                'startDate' => $contract->end_date ? $contract->end_date->format('Y-m-d') : ($contract->start_date ? $contract->start_date->format('Y-m-d') : null),
                                'endDate' => $contract->end_date ? $contract->end_date->format('Y-m-d') : ($contract->start_date ? $contract->start_date->format('Y-m-d') : null),
                ...
                if (in_array('Taskly', $activeModules)) {
                    if ($moduleFilter == "projecttask" || $moduleFilter == 'all') {
                        ...
                        foreach ($tasks as $task) {
                            ...
                            } elseif ($duration && strtotime($duration)) {
                                $startDate = date('Y-m-d', strtotime($duration));
                                $endDate = $startDate;
                            } elseif ($task->milestone && $task->milestone->start_date) {
                                $startDate = $task->milestone->start_date->format('Y-m-d');
                                $endDate = $task->milestone->end_date ? $task->milestone->end_date->format('Y-m-d') : $startDate;
                            } else {
                            ...
                            $events[] = [
                                ...
                                'endDate' => $endDate,
                                'time' => '00:00',
                                'description' => 'Project Task (' . ($task->duration ?? '0') . ')',
                                'event_type' => 'local',
                                'type' => 'projecttask',
                                'color' => '#7c3aed',
                                'raw_id' => $task->id,
